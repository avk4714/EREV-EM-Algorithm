...
for jj = 1:length(numCycle)
    if ~isempty(drvCycle)
        switch choice
            ...
            case 2
                fileName = 'Dec062019154949.mat'; %Dec062019154949 Dec032019165351
                if ispc
...
%     N_OptResults = OptResults;
    ...
    switch simChoice
        ...
        case 3
            INITSOCVEC = 0.1:0.05:1.0;
            for i = 1:length(INITSOCVEC)
                vehParams.SOC_Begin = INITSOCVEC(i);
                tic
                eval(['[OptResults_SOCSweep_',int2str(i),'] = optimize_SHEV_DP(drvCycle,vehParams);'])
                t_sim = toc;
                eval(['OptResults_SOCSweep_',int2str(i),'.optTime_s = t_sim;'])
            end
